...
datapath = 'Data/Planck_PassBands/'
...
#f, T = read_f857(datapath, file)
#ind = np.where((f > 500) & (f < 1200))[0]
#ii = np.where(f < 5000)[0]
...
#Unit_converter(f1, T1, dBdTcmb(f1), dBdTiras(f1), 'K_cmb', '(MJy/sr) 1e-20')
#Unit_converter(f1, T1, dBdTiras(f1), dBdTcmb(f1), '(MJy/sr) 1e-20', 'K_cmb')
